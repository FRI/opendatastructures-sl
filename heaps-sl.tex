...
\index{Kopica}%
\index{Dvojiška Kopica}%
\index{Kopica!Dvojiška}%
...
Prva izvedba kopic uporablja polje, da simuliramo popolno dvojiško 
drevo.
...
Druga implementacija je bazirana na bolj fleksiblinih dvojiških drevesih, ki 
podpirajo #meld(h)# operacijo, ki omogoča vrsti s prednostjo, da obsorbira elemente druge vrste s prednostjo #h#.

\section{#Dvojiška Kopica#: implicitno dvojiško drevo}
\seclabel{DvojiškaKopica}

\index{DvojiškaKopica@#DvojiškaKopica#}%
...
#Dvojiška Kopica# uporablja to tehniko, da implicitno predstavi popolno dvojiško drevo
...
V dvojiški kopici,  je #n# elementov shranjenih v tabeli #a#:
...
\begin{figure}
  ...
  \end{center}
  \caption[Adding to a BinaryHeap]{Dodajanje elementa 6 v dvojiško kopico.}
...
\begin{figure}
  ...
  \end{center}
  \caption[Odstranjevanje iz dvojiške kopice]{Odstranjevanje najmanjšega elementa, 4, iz #Dvojiške Kopice#.}
...
od višine (implicitnega) dvojiškega drevesa. Na srečo je to \emph{popolno}
\index{dvojiško drevo!Popolno}%
\index{Popolno Binrano drevo}%
Dvojiško drevo;  vsak nivo, razen zadnji ima maximalno število vozlišč. Tako, je
...
Naslednji teorem povzame uspešnost #Dvojiške Kopice#.

\begin{thm}\thmlabel{binaryheap}
  #Dvojiška Kopica# implementira #Polje# (s prednostjo). Ignoriramo ceno 
  polja da se poveča #resize()#, Dvojiška Kopica podpira operaciji 
  #dodaj(x)# in #odstrani()# v času $O(\log #n#)$ na operacijo.

  Poleg tega, začnimo s prazno #Dvojiška Kopica#, katero koli zaporedje $m$
...
operacije #merge(h1,h2)#. Operacija #merge(h1,h2)# zlije skupaj kopici #h1 in #h2# tako, da združi vozlišči kopice #h1# in #h2# in vrne
...
Po naši induktivni predpostavki je pričakovana dolžina sprehoda naključnega sprehoda
\[
