...
Zgoščevalnce tabele predstavljajo učinkovito metodo za shranjevanje majhnega števila celih števil
...
V tem primeru je celoštevilska \emph{zguščvelana koda}
\index{razpršena koda}%
povezana z vsako podatkovno enoto in uporabljena v zgoščevalni tabeli. 
Drugi del predstavi, kako so zgoščevalnce kode ustvarjene.
...
\seclabel{hashtable}
Podatkovna struktura zgoščvwlna tabela z veriženjem za shranjevanje tabele t seznamov uporablja zgoščevanje z veriženjem. Za hranjenje skupnega števila podatkov v vseh seznamih se uporablja celo število n.
...
\begin{figure}
   ...
   \end{center}
   \caption[A ChainedHashTable]{Primer verižene zgoščevalne tabele z $#n#=14$ in $#t.length#=16$.  V tem primeru je $#hash(x)#=6$}
...
Pri dodajanju elementa #x#, v zgoščeno tabelo, najprej preverimo če je potrebno povečati #t.length#. V kolikor je to potrebno ga povečamo.
...
Hitrosti razpršenih tabel so odvisne predvsem od izbire zgoščevalne funkcije. Dobra zgoščevalna funkcija razporedi elemente sorezmerno med #t.length# seznamov, tako da je pričakovana
...
Zgoščevanje z množenjem je učinkovita metoda tvorbe zgoščevalnih vrednosti osnovana na modularni aritmetiki(opisana v poglavju  \secref{arrayqueue}) in celoštevilskemu deljenju. Uporablja operator $\ddiv$ , 
...
Pri zgoščevanju z množenjem uporabljamo tabele velikosti $2^{#d#}$ pri čemer je #d# neko celo število(imenovano \emph{dimenzija}). Formula za razprševanje celega števila $#x#\in\{0,\ldots,2^{#w#}-1\}$ je
\[
    #hash#(#x#) = ((#z#\cdot#x#) \bmod 2^{#w#}) \ddiv 2^{#w#-#d#} \enspace .
\]

Pri tem je #z# neko naključno izbrano \emph{celo} število v $\{1,\ldots,2^{#w#}-1\}$.
Zgoščevalna funkcija je lahko realizirana zelo učinkovito, z obzirom na to, da so operacije nad celimi število že v osnovi izbedene nad $2^{#w#}$ biti, kjer je $#w#$ število bitov v celem številu. 
