...
\subsection{Spodnja meja za primerjalne urejevalne algoritme}
...
Sedaj smo videli tri primerjalne urejevalne algoritme, ki imajo časovno zahtevnost $O(#n#\log #n#)$.
...
Ker se elementi ne ponavljajo, ima prva primerjava samo dva možna izida. Druga primerjava pa je
 ...
 lahko kateri koli deterministični primerjalni urejevalni algoritem predstavljamo kot \emph{comparison tree} s korenom.
...
Primerjalno drevo za urejevalni algoritem, nam pove vse o njem. Pove nam natančno zaporedje primerjav,
...
\begin{thm}\thmlabel{deterministic-sorting-lower-bound}
 Za kateri koli deterministični urejevalni algoritem $\mathcal{A}$ , ki temelji na primerjavah, in
...
nam pokaže, da ima vsako binarno drevo s $k$ listi, višino vsaj $\log k$. Posledično mora imeti primerjalno
...
\thmref{deterministic-sorting-lower-bound} govori o determinističnih algoritmih, kot sta razvrščanje z zlivanjem in urejanje s kopico.
...
Na urejevalni algoritem $\mathcal{R}$, ki ima naključnost, lahko gledamo kot deterministični algoritem, ki sprejme
2 vhoda: Polje #a#, ki ga bomo uredili, in dolog zaporedje $b=b_1,b_2,b_3,\ldots,b_m$ naključnih realnih števil
...
Če za $b$ uporabimo neko določeno zapored $\hat{b}$, potem $\mathcal{R}$ postane deterministični urejevalni algoritem,
...
\excref{randomized-lower-bound} zahteva dokaz, da če izberemo naključen list iz binarnega drevesa, ki ima $k$ listov, potem
...
\begin{thm}\thmlabel{randomized-sorting-lower-bound}
 ...
 sortirni algoritem $\mathcal{A}$, je pričakovana vrednost primerjav, ki jih stori algoritem pri naključni permutaciji
...
meji izreka 11.5 \thmref{deterministic-sorting-lower-bound}
z uporabo (nekaterih delov) elementov v #a# kot indeksov v matriki/v nizu.
Razmislite o izrazu
\[
  #c[a[i]]# = 1 \enspace .
\]
Ta izraz se izvrši v konstantnem/stalnem času , ampak ima #c.length# možnih
...
Recimo da imamo vnosno polje #a# sestavljeno iz #n# celih števil, vse v
...
Koda ki to izvrši je zelo spretna, njeno delovanje je ilustrirano v
\figref{countingsort}:
...
Z uporabo vrednosti #a# kot indeks se ti števci lahko vsi izračunajo v času $O(#n#)$ z samo
eno for zanko. Na tej točki bi lahko uporabili #c# da direktno zapolnimo
...
Algoritem urejanje s štetjem  ima prijetno lastnost in sicer da je \emph{stable};
...
#w#/#d#\rceil$.}
Natančneje, korensko urejanje prvo uredi števila po najmanj pomembnih #d# bitih nato po naslednjih #d#
pomembnejših bitih in tako naprej,
v zadnjem prehodu so števila urejena po najpomembnejših #d# bitih
dokler v zadnjem prehodu niso števila urejena po njihovih najpomembnejših d bitih.
...
Primer korakov tega algoritma je prikazan na \figref{radixsort}.
...
stabilen urejevalni algoritem. Če sta $#x# < #y#$ dva elementa polja
