...
\subsection{Razpršilna Koda za Sestavljene Objekte}
\seclabel{stringhash}

\index{hash code!for compound objects}%
Za sestavljen objekt, želimo kreirati razpršilno kodo tako, da združimo
razpršilne kode vseh sestavnih delov objekta. To ni tako preprosto kot se sliši. 
Čeprav lahko najdemo veliko rešitev za ta problem (naprimer, združevanje razpršilnih
kod z bitnimi eksluzivni-ali operacijami),
se izkaže, da se da veliko teh rešitev enostavno zaobiti (glej naloge~\ref{exc:hash-hack-first}--\ref{exc:hash-hack-last}).
Vendar, če lahko naredimo aritmetiko z natančnostjo $2#w#$ bita,
potem imamo na voljo preproste in robustne metode.
Predpostavimo, da imamo objekt sestavljen iz večih delov
$P_0,\ldots,P_{r-1}$ čigar razprševalne kode so $#x#_0,\ldots,#x#_{r-1}$.
Potem lahko izberemo med seboj neodvisne naključne #w#-bitne integerje
$#z#_0,\ldots,#z#_{r-1}$ in naključni $2#w#$-bitni lihi integer #z# in
izračunamo razpršilno kodo za naše objekte s
\[
   h(#x#_0,\ldots,#x#_{r-1}) =  
   \left(\left(#z#\sum_{i=0}^{r-1} #z#_i #x#_i\right)\bmod 2^{2#w#}\right)
   \ddiv 2^{#w#} \enspace .
\]
Upoštevajte, da ima ta razprševalna koda zadnji korak (deljenje z #z# in
deljenje z $2^{#w#}$), ki uporablja multiplikativno razpršilno funkcijo
iz \secref{multihash}, da vzame $2#w#$-bitni vmesni rezultat in
ga pomanjša v #w#-bitni končni rezultat. Tukaj je primer te metode uporabljene na enostavnem sestavljenemu objektu s tremi deli #x0#, #x1#, and #x2#:
...
\begin{Dokaz}
  Najprej bomo ignorirali zadnji multiplikativni razpršilni korak, in pogledali kako
  ...
  Zadnji korak razpršilne funkcije se uporablja za multiplikativno razprševanje
  za zmanjšanje naših $2#w#$-bitnih vmesnih rezultatov $h'(#x#_0,\ldots,#x#_{r-1})$ 
  v #w#-bitni končni rezultat $h(#x#_0,\ldots,#x#_{r-1})$.  Po \thmref{multihash},
