...
#add(i,x)# in #remove(i)#) in kopiranjem (s #resize()#) podatkov.
...
V tem poglavju bomo predtavili podatkovno strukturo #ArrayQueue#, ki implementira FIFO vrsto; elemente z vrste odstranjujemo (z uporabo operacije #remove()#) v istem vrsten redu, kot so bili dodani (z uporabo operacije #add(x)#).

Opazimo, da #ArrayStack# ni dobra izbira za izvedbo FIFO vrste in sicer zato, ker moramo izbrati en konec seznama, na katerega potem dodajamo elemente, nato pa elemente odstranjujemo z drugega konca. Ena izmed operacij mora delovati na glavi seznama, kar vključuje klicanje #add(i,x)# ali #remove(i)#, kjer je vrednost $#i#=0$. To nudi čas izvajanja sorazmeren #n#.
...
Težava te rešitve je, da potrebuje neskočno polje. #ArrayQueue# to simulira z uporabo končnega polja in \emph{modularne aritmetike}.
