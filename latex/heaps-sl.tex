...
drevo.
Ta zelo hitra implementacija je osnova za enega izmed najhitrej?ih 
znanih sortirnih algoritmov, in sicer Kopi?no urejanje. (glej \secref{Kopicno urejanje}).

Druga implementacija je bazirana na bolj fleksiblinih binarnih drevesih, ki 
podpirajo #meld(h)# operacijo, ki omogo?a vrsti s prednostjo, da obsorbira elemente druge vrste s prednostjo #h#.

\section{#Binarna Kopica#: implicitno binarno drevo}
\seclabel{BinarnaKopica}

\index{BinarnaKopica@#BinarnaKopica#}%
Na?a prva implementacija #Vrste# (s prednostjo) temelji na tehniki, 
ki je stara preko 400 let. \emph{Eytzingerjeva metoda}
\index{Eytzingerjeva metoda}%
nam omogo?a, da predstavimo popolno binarno drevo kot polje, v katerem imamo vozli??a 
postavljena v vrsto iz leve proti desni.
(glej \secref{bintree:traversal}).
Na ta na?in je koren drevesa shranjen na poziciji 0, njegov levi otrok je shranjen na poziciji 0, 
...
?e uporabimo Eytzingerjevo metodo na dovolj velikih drevesih se
za?nejo pojavljati vzorci.
Levi otrok vozli??a pri indexu #i# je na indexu $#left(i)#=2#i#+1$ 
in desni otrok vozli??a pri indexu #i# je na indexu $#right(i)#=2#i#+2$
Star? vozli??a pri indexu #i# pa je na $#parent(i)#=(#i#-1)/2$.
...
v katerem so elementi \emph{Kopi?no urejeni}:
\index{heap-ordered binary tree}%
\index{binary tree!heap-ordered}%
\index{heap order}%
Vrednost shranjena na kateremkoli indexu #i# ni manj?a kot vrednost shranjena
na kateremkoli indexu #parent(i)# razen izjeme vrednosti korena $#i#=0$. To nam 
omogo?a, da je najmanj?a vrednost #vrste# s prednostjo tako na shranjena na poziciji 0 (koren).
...
najprej pogledamo, ?e je #a# poln (preverimo $#a.length#=#n#$) in ?e je pove?amo #a#. Nato #x# zapi?emo na mesto #a[n]# in pove?amo #n#. Na tej to?ki je potrebno storiti samo ?e to, da zagotovimo lastnost kopice.
To delamo tako, da zamenjujemo #x# z njegovim star?em dokler ni #x# manj?i od svojega star?a.
...
Implementacija #remove()# operacije, katera odstarani najmanj?o vrednost
v kopici, je nekoliko te?je. Vemo kje je najmanj?i element (v korenu), vendar ga moramo po odstranitvi nadomestiti in zagotoviti, da ohranjamo
lastnosti kopice.

Najla?ji na?in, da to naredimo je, da koren nadomestimo z vrednostjo #a[n-1]#, 
zbri?emo vrednost in zmanj?amo "n". Na ?alost novi koren najverjetneje ni najmanj?i element, zato ga  moramo prestaviti po kopici navzdol. To naredimo tako, da rekurzivno primerjamo element z njegovimi otroki. V primeru, da je element v kopici najmanj?i smo kon?ali, v nasprotnem primeru ga zamenjamo z najmanj?im od njegovih otrok in nadaljujemo z primerjavo.
...
\begin{figure}
  ...
  \end{center}
  \caption[Odstranjevanje iz binarne kopice]{Odstranjevanje najmanjsega elementa, 4, iz #Binarne Kopice#.}
  \figlabel{heap-remove}
\end{figure}


Kot ostale implementirane strukture iz polja, bomo mi ignorirali porabljen ?as
v celicah za funkcijo #povecaj()#, ker se to lahko obra?unava na amortizacijskem argumentu iz Lemma
\lemref{arraystack-amortized}. Prete?eni ?as za #dodaj()# in #remove()# je odvisen
od vi?ine (implicitnega) binarnega drevesa. Na sre?o je to \emph{popolno}
\index{Binarno drevo!Popolno}%
\index{Popolno Binrano drevo}%
Binarno drevo;  vsak nivo, razen zadnji ima maximalno ?tevilo vozli??. Tako, je
vi?ina drevesa enaka $h$ in ima najmanj $2^h$ vozli??. Za?nimo na ta na?in:
\[
  #n# \ge 2^h \enspace .
\]  
Algoritem nam da na obeh straneh ena?be
\[
   h \le \log #n# \enspace .
\]
tako obe, #add(x)# in #remove()# operaciji te?eta v$O(\log #n#)$ ?asu.

\subsection{Povzetek}

Naslednji teorem povzame uspe?nost #Binarne Kopice#

\begin{thm}\thmlabel{binaryheap}
  #Binarna Kopica# implementira #Polje# (s prednostjo). Ignoriramo ceno 
  polja da se pove?a #resize()#, Binarna Kopica podpira operaciji 
  #add(x)# in #remove()# v ca?u $O(\log #n#)$ na operacijo.

  Poleg tega, za?nimo s prazno #Binarno kopico#, katerokoli zapredje $m$
  #add(x)# in #remove()# opraciji je rezultat skupaj $O(m)$ ?as enak
