...
V tem poglavju bomo preučili podatkovno strukturo z binarnim iskalnim drevesom, #ScapegoatTree#. Struktura temelji na znanem dejstvu, da, ko gre nekaj narobe, ljudje najprej nekoga okrivijo (\emph{grešni kozel}).
\index{scapegoat}%
...
Med delno rekonstrukcijo se celotno poddrevo razstavi in zgradi nazaj v popolnoma uravnoteženo poddrevo. Obstaja mnogo načinov, kako spremeniti drevo s korenom v vozlišču #u# v popolnoma uravnoteženo drevo. En najpreprostejših je, da se sprehodimo čez poddrevo #u# in zberemo vsa vozlišča v tabelo #a#, nato pa iz te tabele rekurzivno zgradimo uravnoteženo poddrevo. Če je $#m#=#a.length#/2$, potem je element #a[m]# koren novega poddrevesa, elementi $#a#[0],\ldots,#a#[#m#-1]$ se shranijo rekurzivno v levo poddrevo in $#a#[#m#+1],\ldots,#a#[#a.length#-1]$ se shranijo rekurzivno v desno poddrevo.
...
\section{#ScapegoatTree#: Binarno iskalno drevo z delno rekonstrukcijo}
\seclabel{scapegoattree}


\index{ScapegoatTree@#ScapegoatTree#}%
#ScapegoatTree# je #binarno iskalno drevo#, ki poleg števila #n# vozlišč v drevesu hrani še števec #q#, ki drži zgornjo mejo dovoljenega števila vozlišč.
...
Implementacija #find(x)# je v #ScapegoatTree# narejena s standardnim algoritmom za iskanje v #binarnem iskalnem drevesu# (glej \secref{binarysearchtree}). Njena časovna zahtevnost je sorazmerna z višino drevesa, ki je po (\myeqref{scapegoat-height}) enaka $O(\log #n#)$.

Ko implementiramo operacijo #add(x)#, najprej povečamo #n# in #q# in potem uporabimo običajni algoritem za dodajanje #x# v binarno iskalno drevo; poiščemo #x# in nato dodamo nov list #u# z $#u.x#=#x#$. Tu se nam lahko posreči in globina #u# ne preseže $\log_{3/2}#q#$. V tem primeru smo zadovoljni z rezultatom in ne naredimo nič drugega.
...
\begin{equation}
   ...
   \eqlabel{scapegoat}
\end{equation}
kjer #w.child# predstavlja otroka #w# na poti od korena do #u#. Kmalu bomo dokazali, da grešni kozel obstaja, za zdaj pa to predpostavimo. Ko smo našli grešnega kozla #w#, popolnoma uničimo poddrevo s korenom v #w# in ga ponovno izgradimo kot popolnoma uravnoteženo binarno iskalno drevo. Iz \myeqref{scapegoat} vemo, da že pred vstavljanjem #u#, poddrevo #w# ni bilo polno binarno drevo.
...
Implementacija #remove(x)# v ScapegoatTree je zelo preprosta. Poiščemo element #x# in ga odstranimo z običajnim algoritmom za odstranjevanje vozlišča iz #binarnega iskalnega drevesa#. (To nikoli ne poveča višine drevesa.) V naslednjem koraku znižamo #n#, #q# pa pustimo nespremenjen. Na koncu preverimo, če je $#q# > 2#n#$ in, če je, \emph{ponovno zgradimo celotno drevo} v popolnoma uravnoteženo binarno iskalno drevo in nastavimo $#q#=#n#$.
...
V tej sekciji bomo analizirali pravilnost in amortiziran čas izvajanja operacij na #ScapegoatTree#. Najprej dokažimo pravilnost takoda pokažemo, da ko operacija #add(x)# naredi vozlišče, ki krši pogoj \myeqref{scapegoat-height}, vedno lahko najdemo grešnega kozla:

\begin{lem}
  Naj bo #u# vozlišče višine $h>\log_{3/2} #q#$ v #ScepegoatTree#.
...
Sedaj analiziramo še preostale dele algoritma, ki jih prej nismo upoštevali. Ostala sta dva dela: cena klicev #size(u)#, ko iščemo grešne kozle in cena #rebuild(w)#, ko najdemo grešnega kozla #w#. Cena klicev #size(u)# je povezana s ceno klicev #rebuild(w)# na sledeč način:
...
\subsection{Summary}
Sledeči teorem povzame učinkovitost podatkovne strukture #Scapegoattree#:
...
Poleg tega, če začnemo s praznim #scapegoattree#, poljubno zaporedje #add(x)# in #remove(x)# operacij zahteva kvečjemu $O(m\log m)$ časa za klice #rebuild(u)#.
\end{thm}
