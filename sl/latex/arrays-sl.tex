...
\begin{equation}
  ...
  \eqlabel{das-front}
\end{equation}
Po drugi strani pa, če drži $#i#\ge#front.size()#$, se #add(i,x)# izvede s klicem $#back.add(i-front.size(),x)#$.  Cena tega pa je
\begin{equation}
...
Drugi primer \myeqref{das-back} se pojavi, ko velja $#i#\ge 3#n#/4$.  Kadar $#n#/4\le#i#<3#n#/4$, ne moremo biti prepričani ali delovanje vpliva na #front# ali #back#, ampak v vsakem primeru se postopek izvaja $O(#n#)=O(#i#)=O(#n#-#i#)$ časa, saj je $#i#\ge #n#/4$ in $#n#-#i#>#n#/4$.  Če povzamemo situacijo imamo
\[
...
Če metoda #balance()# izvede uravnovešenje, potem premakne $O(#n#)$ elementov in za to potrebuje $O(#n#)$ časa. To je slabo zato, ker je metoda #balance()# klicana z vsakim #add(i,x)# in #remove(i)# klicem.  V vsakem primeruu, sledeč dokaz dokazuje, da metoda #balance()# v povprečju porabi samo konstantno količino časa na operacijo.

\begin{lem}\lemlabel{dualarraydeque-amortized}
Če ustvarimo prazen #DualArrayDeque# in če katero zaporedje $m\ge 1$ izvede klice   metode #add(i,x)# in #remove(i)#, potem je skupen zapravljen čas med vsemi klici  metode #balance()# $O(m)$.
\end{lem}

\begin{proof}
Dokazali bomo, da če je metoda #balance()# prisiljena premešati elemente, potem je število #add(i,x)# in #remove(i)# operacij vsaj $#n#/2-1$ od kar, so bili elementi nazadnje premešani z metodo #balance()#.
...
Izvedli bomo našo analizo z uporabo tehnike poznane kot \emph{potencialna metoda}.
  ...
  ali #remove(i)#, ki ne opravi nobenega uravnovešenja, lahko poveča potencial skoraj največ za 1.


  Upoštevat je potrebno, da je takoj po klicu metode #balance()#, ki premeša elemente, potencial $\Phi_0$, največ 1, saj
...
Razmislite o trenutku takoj pred klicem funkcije #balance()#, ki premeša elemente in domnevajte, da #balance()# premeša elemente zaradi $3#front.size()# < #back.size()#$.
  ...
  \Phi_1 & = & #back.size()# - #front.size()# \\
      ...
      &=& #n#/2
  \end{eqnarray*}
  Zato je število klicev metode #add(i,x)# ali #remove(i)# od kar, je metoda #balance()# nazadnje premešala emelente, najmanj $\Phi_1-\Phi_0 > #n#/2-1$. To zaključuje dokaz.
\end{proof}
...
\begin{thm}\thmlabel{dualarraydeque}
  #DualArrayDeque# implementira #List# vmesnik. Z ignoriranjem cene klicev metod #resize()# in #balance()#, #DualArrayDeque# podpira operacije
  \begin{itemize}
