...
onceptualno je preskočni seznam  sekvenca  posamezno povezanih seznamov
...
\index{SkiplistList@#SkiplistList#}%
 #SkiplistList# implementira #List# vmesnik s pomočjo(uporabo) preskočnega seznama.
...
Za doseganje tega, potrebujemo možnost iskanja poti
 ...
 \emph{length} od roba nekega seznama, $L_{#r#}$.
...
število robov v $L_0$ below #e#.  Poglej \figref{skiplist-lengths} za
 primer preskočnega seznama z dolžino njegovih robov.  Since the
Posledica shranjevanja robov preskočnega seznama v nizih, lahko dolžino shranjujemo na enak 
način:
...
ki se nahaja na poziciji #j# v $L_0$ in sledimo robu 
dolžine $\ell$, then we move to a node whose position, in $L_0$,
is $#j#+\ell$.  Po takem postopku, medtem ko iščemo iskalno pot lahko ohranjamo
vrednost pozicije, #j#, trenutnega vozlišča v $L_0$.  Medtem ko na vozlišču,
 #u#, v $L_{#r#}$, gremo desno če #j# plus dolžina  roba
...
Ker je najtežji del operacij #get(i)# in #set(i,x)# iskanje
 #i#th vozlišča v $L_0$, se operacije izvedejo v
$O(\log #n#)$ časa.
...
tse dolžina roba #u.next[r]# poveča za ena, ker dodajamo element pod rob na poziciji #i#.
...
\begin{figure}
  ...
  \end{center}
  \caption[Adding to a SkiplistList]{Posodabljanje dolžine robov, medtem še spajanje vozlišče  
...
\begin{figure}
  ...
  \end{center}
  \caption[Removing an element from a SkiplistList]{Brisanje elementa od #SkiplistList#.}
