...
drevo.
To zelo hitra implementacija je osnova za enega izmed najhitrejših 
znanih sortirnih algoritmov, in sicer Kopièno urejanje. (see \secref{Kopièno urejanje}).

Druga implementacija je bazirana na bolj fleksiblinih binarnih drevesih, ki 
podpira #meld(h)# operacijo, ki omogoèa vrsti s prednostjo da obsorbira elemente druge vrste s prednostjo #h#.

\section{#BinarnaKopica#: inplicitno binarno drevo}
\seclabel{BinarnaKopica}

\index{BinarnaKopica@#BinarnaKopica#}%
Naša prva implementacije #Vrste# (s prednostjo) temelji na tehniki, 
...
nam omogoèa da predstavimp popolno binarno drevo kot polje, v katerem imamo vozlišèa 
postavlna v vrsto iz leve proti desni.
(glej \secref{bintree:traversal}).
Na ta naèin je koren drevesa spravljen na pozociji 0, njegov levi otrok je shranjen na poziciji 0, 
...
Èe uporabimo Eytzingerjevo metodo na zadosti velikih drevesih se
zaènejo pojavljati vzroci.
Levi otrok vozlièa pri indexu #i# je na indexu $#left(i)#=2#i#+1$ 
in desni otrok vozlišèa pri indexu #i# je na indexu $#right(i)#=2#i#+2$
Starš vozlišèa pri indexu #i# pa je na $#parent(i)#=(#i#-1)/2$.
...
v katerem so elementi \emph{Kopièno urejeni}:
\index{Kopièno urejeno binarno drevo}%
\index{Binarno Drevo!Kopièno urejeni}%
\index{Kopièno urejanje}%
Vrednost shranjena na kateremkoli indexu #i# ni manjša kot vrednost shranjena
na kateremkoli indexu #parent(i)# razen izjeme vrednosti korena $#i#=0$. To nam 
omogoèa da je najmanjša vrednost #vrste# s prednostjo tako na shranjena na poziciji 0(koren).
...
najprej pogledamo èe je #a# pol (preverimo $#a.length#=#n#$) in èe je poveèamo #a#
#a[n]# in increment #n#. Na tej poti je samo še kar nam ostane, da zagotovimo lastnost kopice.
To delamo tako da premešèamo #x# z njegovim staršem dokler ni #x# manjši od svojega starša.
...
Implementavija #remove()# operacije, katera odstarani najmanjšo vrednost
v kopici, je malo težje. Vemo kje je najmanjši element (v korenu), ampak 
ga moramo nademstiti potem ko ga odstranimi in zagotoviti to ohranjamo
lastnosti kopice.

Najlažji naèin da to naredimo da koren nadomestimo v vrednostjo #a[n-1]#, 
zbrišemo vrednost  in decrement "n". Na žalost novi koren najverjetneje ni najmanjši
element, zato ga  moramo prestaviti dol v kopici. To naredimo tako da ponavljajoèe
primerjamo ta element z njegovimi otroki. Èe je najmanjši v kopici smo koèali, 
v nasprotnem primeru ga zamenjamo z najmanjšim od njegovih otrok in nadaljujemo 
z primerjavo.
...
\begin{figure}
  ...
  \end{center}
  \caption[Odstranjevanje iz binarne kopice]{Odstranjevanje 4 najmanjšega elementa, iz #Binarne Kopice#.}
  \figlabel{heap-remove}
\end{figure}


Kot ostali iz polja implementirane strukture, bomo mi ignorirali èas porabljen
v celicah za #poveèaj()#, ker se to lahko obraèunava na amortizacijskem argumentu iz
\lemref{arraystack-amortized}. Pretekli èas za #Dodaj()# in #remove()# je odvisen
od višine (implicitnega) binarnega drevesa. Na sreèo je to \emph{popolno}
\index{Binarno drevo!Popolno}%
\index{Popolno Binrano drevo}%
Binarno drevo;  vsako nivo razen zadnje ima maximalno število vozlišè. Tako, je
višina drevesa enaka $h$ in ima najmanj $2^h$ vizlišè.

zaènimo na ta naèin:
\[
  #n# \ge 2^h \enspace .
\]  
Algoritem da na obeh straneh enaèbe
\[
   h \le \log #n# \enspace .
\]
teko obe #add(x)# in #remove()# operaciji teèeta v$O(\log #n#)$ èasu.
...
\begin{thm}\thmlabel{binaryheap}
  ...
  polja da se poveèa#resize()#, #Binarna Kopica podpira oberaciji 
  #add(x)# in #remove()# v èasu $O(\log #n#)$ na operacijo.

  Tako naprej, zaèetek z prazno #Binarno kopico#, katerokoli zapredje $m$
  #add(x)# in #remove()# opraciji je rezultat skupaj $O(m)$ èas enak
  porabljen za poveèanje #resize()#.
\end{thm}
