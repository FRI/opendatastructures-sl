...
lastnosti. Vsaka lastnost je definirana v obeh izrazih, v rdeči in črni barvi in številski vrednostih 0 in 1.
...
\noindent
Primer 3: sin vozlišča #u# je črn in #u# je desni sin #w#. Primer je simertričen Primeru 2 in ga rešujemo precej podobno. Razlikuje se v tem, da je lastnost levo-visečega asimetrična, in zato ga obravnavamo drugače.

Kot pri prejšnjem, začnemo s klicem #pullBack(w)#, kar naredi #v# rdeče vozlišče in #u# črno. S klicem #flipRight(w)# postane #v# koren našega poddrevesa. Tako je #w# rdeč, zato sedaj ločimo dva primera glede na #q#, ki je levi sin #w#.
...
Bolj zapleteno pa je v primeru, ko je #q# črne barve. Tu moramo preveriti barvo levega sina vozlišča #v#. Če je ta rdeč, potem ima #v# dva rdeča sinova, zato pokličemo #pushBlack(v)#. Sedaj je #w# črn, #v# je prejšnje barve #w# in smo končali z urejanjem.

Če je pa levi sin #v# črn, kršimo lastnost levo-visečega. Vspostavimo jo nazaj s klicem #flipLeft(v)#. Nato vrnemo vozlišče #v#, zato da se naslednja iteracija #removeFixup(u)# nadaljuje z $#u#=#v#$.
...
od 2-4 dreves. Ta so veliko preprostejša kot podatkovna struktura #rdeče-črnih dreves# 
...
\index{AVL tree}%
AVL drevesa so \emph{višinsko poravnana}:
...
\emph{zlato razmerje}.  (Bolj natančno $|\varphi^h/\sqrt{5} - F(h)|\le 1/2$.)
...
AVL dreves je enostavnejša kot uporaba strukture #rdeče-črnih dreves#.
Žal pa ne more nobena od njih zagotavljati, da bi bil amortizacijski čas $O(1)$, 
za vsako posodobitev uravnovešen. Zlasti zato,
ker te strukture nemoremo primerjati z of \thmref{redblack-amortized}.
...
\begin{exc}
  Nariši 2-4 drevo, ki ustreza #rdeče-črnemu drevesu# iz
  \figref{redblack-example2}.
\end{exc}

\begin{exc}
  Nariši dodajanje elementov 13, 3.5 in 3.3 na #rdeče-črno drevo#
  iz \figref{redblack-example2}.
\end{exc}

\begin{exc}
  Nariši odstranjevanje elementov 11, 9, ter 5 na #Rdeče-črnem drevesu# iz 
  \figref{redblack-example2}.
\end{exc}
...
\begin{exc}
  Zakaj metoda #odstrani(x)# v #rdeče-črnem drevesu# izvede operacijo
...
\begin{exc}
  ...
  zgoraj.  Primerjaj hitrost izvajanja s hitrostjo #rdeče-črnega drevesa#.
