...
Potem zgostimo #x#, da dobimo število #i#, v razponu $\{0,\ldots,#t.length#-1\}$, in pripnemo #x# seznamu #t[i]#:
...
Hitrosti zgoščevalnih tabel so odvisne predvsem od izbire zgoščevalne funkcije. Dobra zgoščevalna funkcija razprši elemente enakomerno med #t.length# seznamov, tako da je pričakovana
...
\section{#LinearHashTable#: Odprto naslavljanje}
\seclabel{linearhashtable}
\translatedby{Igor Plavšić}{sl}
\translatedby{Edin Beganović}{sl}
...
z zgoščevalno vrednostjo #i=hash(x)# v lokacijo tabele #t[i]#. Če tega ne moremo 
...
tako, da je bodisi #t[i']=x#, ali #t[i']=null#. V prvem primeru bomo vrnili  #t[i']#. V drugem primeru pa lahko ugotovimo, da x ni vsebovan v zgoščevalni tabeli in vrnemo #null#.
...
\begin{proof}
  ...
  Zadnji korak zgoščevalne funkcije se uporablja za multiplikativno zgo-šče-vanje, 
...
To je zaradi tega, ker ta funkcija učinkovito zgoščuje neskončno sekvenco
\[
...
daljša dolžino $r$, da imata enako zgoščeno vrednost v najslabšem primeru
\[
    2/2^{31} + r/(2^{32}-5)
\]
...
Zgoščevalne tabele in zgoščene vrednosti predstavljajo aktivno področje raziskovanja, ki se ga v tem poglavju le grobo dotaknemo.
Spletna bibliografija o zgoščevanju \cite{hashing} \index{Bibliography on Hashing}
 vkjučuje skoraj 2000 vnosov.

Obstaja veliko različnih implementacij zgoščevalnih tabel. Metodo opisano v
 \secref{hashtable} poznamo pod imenom \emph{zgoščevanje z veriženjem}
\index{hashing with chaining}
(vsak vnos v tabelo vsebuje verigo (#sezman#) elementov).  
Korenine zgoščevanja z veriženjem segajo vse do internega pisma v podjetju IBM katerega
je januarja 1953 napisal H. P. Luhn. To pismo je tudi ena izmed prvih referenc na povezane sezname.

\index{open addressing}
Alternativa zgoščevanju z veriženjem se uporablja pri \emph{odprtem naslavljanju}, kjer so vsi podatki
shranjeni neposredno v tabeli. Sem spadajo strukture iz družine #LinearHashTable# 
\secref{linearhashtable} . To idejo je prav tako predlagala neka druga (nepovezana) skupina pri podjetju 
IBM okoli leta 1950.  Sheme odprtega naslavljanja morajo nasloviti težave \emph{odpravljanja trkov} : 
\index{collision resolution}
primer kadar se dve vrednosti preslikata v enako lokacijo v tabeli. 
Obstajajo različne tehnike odpravljanja trkov. Te ponujajo različne zmogljivosti in velikokrat uporabljajo bolj napredne
zgoščevalne funkcije kot tiste, ki so opisane tukaj.

Obstaja še ena metoda izvedbe zgoščevalnih tabel - tako imenovane
\emph{popolne zgoščevalne} metode.
\index{perfect hashing}
To so metode kjer #find(x)# operacije
vzamejo $O(1)$ časa v najslabšem primeru.  Za statične podatke lahko do tega pridemo z iskanjem
 \emph{popolne zgoščevalne funkcije}
\index{perfect hash function}
\index{hash function!perfect}
za te podatke. Te funkcije preslikajo vse podatke na unikatno mesto v tabeli.
Za podatke, ki se skozi čas spreminjajo poznamo dve metodi popolnega zgoščevanja - \emph{FKS dvonivojske zgoščevalne tabele}
\index{two-level hash table}
\index{hash table!two-level}
\cite{fks84,dkkmrt94}
in \emph{cuckoo hashing} \cite{pr04}.
\index{cuckoo hashing}
\index{hash table!cuckoo}

Zgoščevalne funkcije predstavljene v tem poglavju so verjetno ene izmed najbolj
praktičnih metod (ki jih poznamo) za shranjevanje vseh vrst podatkov.
Ostale dobre metode segajo nazaj do prelomnega dela Carterja in Wegmana, ki 
sta predstavila idejo \emph{univerzalnega zgoščevanja}
\index{universal hashing}
\index{hashing!universal}

in opisala različne zgoščevalne funkcije za različne scenarije \cite{cw79} .
Tabelarično zgoščevanje, opisano v \secref{tabulation} , poznamo po zaslugi Carterja
in Wegmana \cite{cw79} . Analizo uporabe tabelaričnega zgoščevanja pri linearnem poizvedovanju
(in nekaj ostalih metodah) pa sta opisala P\v{a}tra\c{s}cu in Thorup \cite{pt12} .
...
$\{0,\ldots,2^{#w#}\}$. To ustvari $2#w#$-bitno zgoščeno vrednost, katere možnost kolizije je $1/2^{#w#}$.  To se lahko zmanjša na #w#-bitno zgoščeno vrednost z uporabo zgoščevanja z množenjem. Ta metoda je hitra, ker zahteva samo $r/2$ $2#w#$-bitnih množenj, metoda omenjena v \secref{stringhash} pa zahteva $r$ množenj.
...
\begin{exc}
  Upoštevajte spodnjo poenostavljeno verzijo kode za dodajanje elementa #x# v #LinearHashTable# (zgoščevalno tabelo z odprtim naslavljanjem), ki element #x# shrani v
...
\begin{exc}
  ...
  niz se je zgoščena vrednost izračunala glede na osem sodo indeksiranih znakov. 
  Na primeru pojasnite zakaj to ni bila pametna ideja. Primer naj sestoji
  iz večjega nabora nizov, pri čemer naj imajo vsi enako zgoščeno vrednost.
\end{exc}

\begin{exc}\exclabel{hash-hack-first}
  Predpostavite da imate objekt sestavljen iz dveh #w#-bitnih števil, #x# in #y#. 
  Pokažite zakaj $#x#\oplus#y#$ ni dobra zgoščena vrednost za vaš objekt. Pokažite tudi 
  primer večje množice objektov, kjer bi vsi imeli kodo raz-pr-ši-tve 0.
\end{exc}

\begin{exc}
  Predpostavite da imate objekt sestavljen iz dveh #w#-bitnih števil, #x# in #y#. 
  Pokažite zakaj $#x#+#y#$ ni dobra zgoščena vrednost za vaš objekt. Pokažite tudi 
  primer večje množice objektov, kjer bi vsi imeli enako zgoščeno vrednost.
\end{exc}

\begin{exc}\exclabel{hash-hack-last}
  Predpostavite da imate objekt sestavljen iz dveh #w#-bitnih števil, #x# in #y#. Predpostavite
  tudi, da je zgoščena vrednost za vaš objekt definirana z deterministično funkcijo $h(#x#,#y#)$, ki ustvari eno samo #w#-bitno število. Dokažite da obstaja večja množica objektov, ki 
  imajo enako zgoščena vrednost.
\end{exc}
