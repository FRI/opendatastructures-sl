...
Ena od glavnih razlik med podatkovnima strukturama $B$-dreves in
#binarnimIskalnimDrevesom#(\secref{binarysearchtree}) je, da vozlišča
...
da sta operaciji #add(x)# in #remove(x)# v $B$-drevesih najlažje
implementirani s pomočjo rekurzije.

Kot za vsa iskalna uravnotežena drevesa, je potrebna neka sprememba oblike, 
...
iz 3 na $B+4$ dostope. To bi poslabšalo učinkovitost $B$-drevesa pri 
večjih številih $B$.
...
\begin{description}
  \item[faza ugrezanja:]
    ...
    časa v zunanjem spominskem modelu in $O(\log #n#)$ časa v RAM modelu.
...
Spomnimo se, da je lahko vrednost $B$ precej velika, veliko večja
...
Kakorkoli že, vsaka operacija združevanja in izposojanja potrebuje $O(B)$ časa v word-RAM modelu, 
...
Do sedaj smo pokazali, da 
\begin{enumerate}
  \item V modelu zunanjega pomnilnika je časovna zahtevnost operacij #find(x)#,
    #add(x)#, in #remove(x)# v $B$-drevesu $O(\log_B #n#)$.
  \item V word-RAM modelu je časovna zahtevnost operacije #find(x)# $O(\log #n#)$, 
časovna zahtevnost operacij #add(x)# in #remove(x)# pa je $O(B\log #n#)$.
\end{enumerate}
...
Namen dokaza \lemref{btree-split} je pokazati, da je pri word-RAM modelu časovna zahtevnost operacij
...
metod #add(x)# in #remove(x)# v word-RAM modelu $O(B+\log #n#)$.
...
\begin{thm}[Word-RAM $B$-Drevesa]
  Razred #BTree# implementira vmesnik #SSet#. V word-RAM modelu podpira razred #BTree# operacije
