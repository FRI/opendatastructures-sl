...
To delamo tako, da zamenjujemo #x# z njegovim staršem dokler ni #x# manjši od svojega starša.
...
Kot ostale implementirane strukture iz polja, bomo mi ignorirali porabljen čas
v celicah za funkcijo #povecaj()#, ker se to lahko obračunava na amortizacijskem argumentu iz Lemma
\lemref{arraystack-amortized}. Pretečeni čas za #dodaj()# in #remove()# je odvisen
...
\begin{thm}\thmlabel{binaryheap}
  ...
  #add(x)# in #remove()# v caču $O(\log #n#)$ na operacijo.

  Poleg tega, začnimo s prazno #Binarno kopico#, katerokoli zapredje $m$
  #add(x)# in #remove()# opraciji je rezultat skupaj $O(m)$ čas enak
