...
\begin{thm}\thmlabel{binaryheap}
  #BinaryHeap# implementira #Queue# (s prednostjo). Če ignoriramo ceno 
  #resize()# za povečanje polja, #BinaryHeap# izvede operaciji 
  #add(x)# in #remove()# v času $O(\log #n#)$ na operacijo.

  Poleg tega, začenši s prazno #BinaryHeap#, katero koli zaporedje $m$ operacij
  #add(x)# in #remove()# potrebuje skupno $O(m)$ časa za vse klice funkcije #resize()#.
\end{thm}
...
Operaciji #add(x)# in #remove()# v #MeldableHeap# sta implementirani z uporabo operacije
#merge(h1,h2)#. Operacija #merge(h1,h2)# zlije skupaj kopici#h1# in #h2# tako, da združi
vozlišči kopice #h1# in #h2# in vrne korensko vozlišče nove kopice, ki vsebuje vse elemente poddreves
vozlišč #h1# in #h2#.
