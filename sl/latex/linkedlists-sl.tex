...
#SEList# ima zelo tesne omejitve glede števila elementov v bloku. Razen zadnjega bloka vsebujejo najmanj $#b#-1$ and at most $#b#+1$ elementov. To pomeni, če #SEList# vsebuje #n# elementov, ima največ
\[
    #n#/(#b#-1) + 1 = O(#n#/#b#)
\]
...
Odstranjevanje elementa iz podatkovne strukture #SEList# je podobno dodajanju elementov.
Najprej lociramo vozlišče u, ki vsebuje element z indeksom #i#. Zdaj moramo biti pripravljeni na primer, ko  elementa ne moremo zbrisati iz vozlišča #u#, ne da bi #u#-jev blok postal manjši kot $#b#-1$.

Ponovno naj vozlišča $#u#_0,#u#_1,#u#_2,\ldots$ označujejo #u#, #u.next#, #u.next.next# in tako naprej. Med vozlišči poiščemo tisto vozlišče, iz katerega si lahko sposodimo element, s katerim bo velikost bloka vozlišča $#u#_0$ vsaj $#b#-1$. To lahko storimo na 3 načine (\figref{selist-remove}):
...
\item Hitro (v $r+1\le #b#$ korakih) tečemo s konca seznama blokov. V tem primeru je $#u#_r$ zadnji blok, tako da zanj ni nujno, da vsebuje vsaj $#b#-1$ elementov.  Zato
nadaljujemo kot zgoraj, da si sposodimo element iz $#u#_r$ in iz njega naredimo dodaten element v $#u#_0$.  Če blok vozlišča $#u#_r$ zaradi te menjave postane prazen, 
ga odstranimo.
...
$#b#-1$ elementov. V tem primeru je $#u#_0,\ldots,#u#_{#b#-1}$ zaporedje blokov #b#, kjer vsak izmed njih vsebuje $#b#-1$ elementov.  Teh $#b#(#b#-1)$ elementov \emph{združimo}
 v $#u#_0,\ldots,#u#_{#b#-2}$, tako da vsak izmed teh
...
imata enak čas izvajanja, $O(#b#+\min\{#i#,#n#-#i#\}/#b#)$, če ne poštevamo cene metode
#gather(u)#, ki jo uporabimo v 3. načinu odstranjevanja.

\subsection{Amortizirana analiza širjenja in združevanje}

Razmislimo o ceni metod #gather(u)# in #spread(u)#), 
...
$O((#b#+1)^2)=O(#b#^2)$. Ne glede na vse, naslednji izrek pokaže, da se metodi izvršita na največ enem izmed mnogih #b# klicev metod #add(i,x)# ali #remove(i)#.
...
Uporabili bomo potencialno metodo amortiziranih analiz.
  ...
  Če se izvrši scenarij 3, se izvrši, ker so vsa vozlišča  $#u#_0,\ldots,#u#_{#b#-1}$
  ranljiva. Nato se pokliče metoda $#spread(#u_0#)#$, ki #b# ranljivih vozlišč zamenja z $#b#+1$ robustnimi vozlišči. Na koncu v blok vozlišča $#u#_0$ dodamo #x#, ki vozlišče naredi ranljivo. V splošnem se potencial zniža za $#b#-1$.
...
\begin{thm}\thmlabel{selist}
  Podatkovna struktura #SEList# implementira #List# vmesnik. Kljub ignoriranju stroška klicev metod #spread(u)# in #gather(u)#, #SEList# z #b# velikostjo bloka podpira operacije
  \begin{itemize}
    \item #get(i)# in #set(i,x)# v času $O(1+\min\{#i#,#n#-#i#\}/#b#)$ na operacijo; in
    \item #add(i,x)# in #remove(i)# v času $O(#b#+\min\{#i#,#n#-#i#\}/#b#)$ na operacijo.
  \end{itemize}
  Če začnemo s praznim #SEList#, bo skupno porabljen čas med vsemi klici metod #spread(u)# and #gather(u)# za vsako ponovitev od $m$ #add(i,x)# in #remove(i)# operacij enak $O(#b#m)$.
