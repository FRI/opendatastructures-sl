...
Pojem zunanjega pomnilnika vključuje veliko število različnih naprav, od katerih ima vsaka svojo velikost bloka in je dostopna s svojo zbirko sistemskih klicev. Da poenostavimo razlago tega poglavja in se osredotočimo na skupne ideje, povzamemo zunanje pomnilniške naprave z objektom #blocna shramba#. #Blocna shramba# hrani zbirko spominskih blokov, kjer ima vsak velikost $B$. Vsak blok je enolično določen s celoštevilskim indeksom. #Blocna shramba# podpira sledeče operacije:
...
#Blocno shrambo# si najlažje predstavljamo tako, da si ga zamislimo kot shranjevanje datoteke na disk, kateri je razdeljen na bloke, kjer vsak vsebuje $B$ bajtov. Na ta način #readBlock(i)# in #writeBlock(i,b)# preprosto bereta in zapisujeta bajte $#i#B,\ldots,(#i#+1)B-1$ te datoteke. Poleg tega bi preprosta #blocna shramba# lahko vodila \emph{prosti seznam} blokov, ki so na voljo za uporabo. Bloki, sproščeni s #freeBlock(i)#, so dodani prostemu seznamu. Na ta način lahko #placeBlock(b)# uporabi blok iz prostega seznama ali, če nobeden ni na voljo, doda nov blok na konec datoteke.
...
Implementacija operacije #najdi(x)#, ilustrirana v 
...
#u.otroci[i]#. Če noben ključ v #u.keys# ni večji od #x#, potem iskanje
...
pomnilniškem modelu čas, ki ga porabi operacija #najdi(x)#
$O(\log_B #n#)$. Da določimo čas delovanja v RAM modelu,
moramo računati čas klicanja operacije #najdiIt(a,x)# za vsako vozlišče,
ki ga obiščemo. Čas delovanja operacije #najdi(x)# v modelu besedni RAM je
\[
...
Ena glavnih razlik med podatkovnima strukturama $B$-dreves in
