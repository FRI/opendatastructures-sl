...
in se tako vrnemo na #u.parent#. Naslednja koda izvaja idejo, ki vključuje ravnanje v primerih, ko katera koli od #u.left#,
...
#BinarySearchTree# je posebna oblika binarnega drevesa, pri katerem vsako vozlišče #u# hrani tudi vrdnost #u.x# iz nekega skupnega vrstnega reda. Vrednosti binarnega iskalnega drevesa upoštevajo \emph{lastnost binarnih iskalnih dreves}:
\index{binary search tree property}%
Za vozlišče #u# velja, da je vsaka vrednost, shranjena v poddrevesu #u.left#, manjša od #u.x# ter vsaka vrednost, shranjena v poddrevesu #u.right#, večja od #u.x#.  Primer #BinarySearchTree# je prikazan v \figref{bst}.
...
Lastnost binarnega iskalnega drevesa je zelo uporabna, ker nam omogoča hitro iskanje vrednosti #x# v binarnem iskalnem drevesu. Z iskanjem vrednosti #x# začnemo v korenu #r#. Ko pregledamo vozlišče #u#, imamo tri možnosti:
\begin{enumerate}
\item Če velja $#x#< #u.x#$, nadaljujemo z iskanjem v #u.left#;
\item Če velja $#x#> #u.x#$, nadaljujemo z iskanjem v #u.right#;
\item Če velja $#x#= #u.x#$, potem smo našli vozlišče #u#, ki hrani vrednost #x#.
\end{enumerate}
Iskanje se zaključi v Možnost~3, saj najdemo #x#, ali če je #u=nil#, kar pomeni, da #x# ni v binarnem iskalnem drevesu.
\codeimport{ods/BinarySearchTree.findEQ(x)}

V \figref{bst-search} sta prikazana primera iskanj v binarnem iskalnem drevesu. Drugi primer prikazuje, da četudi ne najdemo #x# v drevesu, vseeno pridobimo nekaj pomembnih informacij. Če pogledamo zadnje vozlišče #u# (Možnost~1) vidimo, da je #u.x# najmanjša vrednost v drevesu, ki je večja od #x#.  Podobno, zadnje vozlišče (Možnost~2) hrani največjo vrednost v drevesu, ki je manjša od #x#.  Torej, ob spremljanju zadnjega vozlišča #z# (Možnost~1), lahko #BinarySearchTree# implementira funkcijo #find(x)#, ki vrne najmanjšo vrednost v drevesu, ki je večja ali enaka #x#:
...
\begin{figure}
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Primer (a)~uspešnega (za $6$) ter (b)~neuspešnega iskanja (za $10$) v binarnem iskalnem drevesu.}
  \figlabel{bst-search}
\end{figure}
...
Primer je prikazan v \figref{bst-insert}. Najbolj časovno potraten del tega procesa je začetno iskanje #x#-a, kjer je porabljen čas sorazmeren z višino novo vstavljenega vozlišča #u#. V najslabšem primeru je čas enak višini #BinarySearchTree#.
