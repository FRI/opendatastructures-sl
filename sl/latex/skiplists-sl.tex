...
#List#časovne zahtevnosti operacij  #get(i)#, #set(i,x)#,
#add(i,x)#, in #remove(i)# $O(\log n)$. Prav tako lahko izvaja #SSet# v katerem so vsi postopki vodijo v $O(\log #n#)$ pričakovanega časa.
...
<<<<<<< .mine
onceptualno je preskočni seznam  zaporedje  posamezno povezanih seznamov
...
Konceptualno je preskočni seznam zaporedje enojno povezanih seznamov $L_0,\ldots,L_h$.Vsak seznam $L_r$ vsebuje podniz elementov v $L_{r-1}$. Začnimo z vhodnim seznamom $L_0$, ki vsebuje #n# elementov in naredimo $L_1$ iz $L_0$, $L_2$ iz $L_1$, in tako naprej.
...
Na koncu vsakega seznama je posebno vozlišče, imanovano \emph{stražar} \index{sentinel node}%
, ki predstavlja statista za seznam. Glavna lastnost preskočnega seznama je, da obstaja kratka pot, imanovana \emph{pot iskanja}, 
...
začnemo pri stražarju, #w#, v $L_h$.  Pregledamo #w.next#.
...
Rešitev, ki si jo bomo podrobneje pogledali v  \secref{skiplist-analiza},
...
<<<<<<< .mine
V naslednjih dveh podpoglavjih  bomo govorili o dveh različnih uporabah preskočnih seznamov.
...
Preden lahko dodamo element v #SkipListSSet#, potrebujemo metodo, ki nam bo simulirala met kovanca za določitev višine,
 #k#, novega vozlišča.
To naredimo tako, da si izberemo poljubno število, #z#, in štejemo število zaporednih enic z desnega konca $1$s v dvojiškem zapisu števila
...
=======
Preden lahko dodamo element v #SkipListSSet#, potrebujemo metodo, ki nam bo simulirala met kovanca za določitev višine #k# novega vozlišča. To naredimo tako, da si izberemo poljubno število #z# in štejemo število zaporednih enic v dvojiškem zapisu števila  #z#:\footnote{Ta metoda ne ponazarja popolnoma eksperiment metanja kovanca saj bo vrednost #k# vedno manjša od števila bitov v #int#. Kakorkoli, to bo imelo malenkosten vpliv dokler ne bo število elementov v strukturi veliko večje kot $2^{32}=4294967296$.}
>>>>>>> .r566

\codeimport{ods/SkiplistSSet.pickHeight()}

<<<<<<< .mine
Pri izvedbi metode #add(x)# v #SkiplistSSet# smo najprej poiskali #x#
...
<<<<<<< .mine
#SkiplistSSet# je uporabljen za izvedbo vmesnika #SSet#.  #SkiplistSSet# opravi  
operacije #add(x)# (dodaj), #remove(x)# (odstrani), and #find(x)# (najdi) v pričakovanem času  $O(\log #n#)$
 na operacijo.
=======
#SkiplistSSet# je uporabljen za izvedbo vmesnika #SSet#. #SkiplistSSet# opravi  operacije #add(x)# (dodaj), #remove(x)# (odstrani) in #find(x)# (najdi) v $O(\log #n#)$  pričakovanega časa za operacijo.
>>>>>>> .r566
\end{thm}
...
številu nivojev v $L_0$, ki so pod #e#.  Poglej \figref{skiplist-lengths} za
 primer preskočnega seznama z dolžino njegovih nivojev.  Ker so nivoji preskočnega seznama shranjeni v polju, lahko na enak način      shranjujemo tudi dolžino:

\begin{figure}
  \begin{center}
    \includegraphics[width=\ScaleIfNeeded]{figs/skiplist-lengths}
  \end{center}
  \caption{Dolžine nivojev v preskočnem seznamu.}
...
Dodajanje elementa v #SkiplistList# na pozicijo, #i#, je enostavno.
  Za razliko od dodajanja v #SkiplistSSet#, vemo da bo vozlišče
...
\begin{figure}
  ...
  \end{center}
  \caption[Adding to a SkiplistList]{Dodajanje v #SkiplistList#.}
...
\begin{proof}
 Verjetnost, da je  kateri koli element, #x#, vsebovan v seznamu
  ...
  je $#n#/2^{#r#}$.\footnote{Poglej \secref{randomization} za obrazložitev kako pridemo do rezultata z uporabo indikatorja spremenljivk in linearnosti pričakovanja.}
...
{Bodite pozorni da lahko pride do 
