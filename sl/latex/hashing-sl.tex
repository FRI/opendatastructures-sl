...
Prvi del poglavja se osredotoča na dve najbolj pogosti implementaciji: razprševanje z veriženjem in linearno naslavljanje.
...
\seclabel{hashtable}
Podatkovna struktura verižena razpršena tabela za shranjevanje tabele t listov uporablja zgoščevanje z veriženjem. Za hranjenje skupnega števila podatkov v vseh seznamih se uporablja celo število n.
...
\begin{figure}
   ...
   \end{center}
   \caption[A ChainedHashTable]{Primer verižene razpršene tabele z $#n#=14$ in $#t.length#=16$.  V tem primeru je $#hash(x)#=6$}
   \figlabel{chainedhashtable}
\end{figure}

\emph{Razpršena vrednost} podatkovnega objekta #x#, označena z #hash(x)# predstavlja vrednost v razponu $\{0,\ldots,#t.length#-1\}$. Vsi podatki z razpršeno vrednostjo #i#
...
Performanse razpršene tabele so odvisne predvsem od izbire razpršilne funkcije. Dobra rapršilna funkcija razporedi elemente sorezmerno med #t.length# seznamov, tako da je pričakovana
velikost seznama #t[hash(x)]# $O(#n#/#t.length)# = O(1)$. Po drugi strani pa slaba razpršilna funkcija razprši vse vrednosti(vključno z #x#) na isto lokacijo v tabeli. V tem primeru bo velikost
seznama #t[hash(x)]# #n#. V naslednjem poglavju je opisan primer dobre zgoščevalne funckije. 

\subsection{Množilno razprševanje}
\seclabel{multihash}

\index{hashing!multiplicative}%
\index{multiplicative hashing}%

Množilno razprševanje je učinkovita metoda tvorbe razpršenih vrednosti osnovana na modularni aritmetiki(opisana v poglavju  \secref{arrayqueue}) in celoštevilskemu deljenju. Uporablja $\ddiv$ operator, 
...
Pri množilnem razprševanju uporabljamo tabele velikosti $2^{#d#}$ pri čemer je #d# neko celo število(imenovano \emph{dimenzija}). Formula za razprševanje celega števila $#x#\in\{0,\ldots,2^{#w#}-1\}$ je
\[
    #hash#(#x#) = ((#z#\cdot#x#) \bmod 2^{#w#}) \ddiv 2^{#w#-#d#} \enspace .
\]

Pri tem je #z# neko naključno izbrano \emph{celo} število v $\{1,\ldots,2^{#w#}-1\}$.
Razprševalna funkcija je lahko realizirana zelo učinkovito, z obzirom na to, da so operacije nad celimi število že v osnovi modulo $2^{#w#}$, kjer je $#w#$ število bitov v celem številu. 
...
\begin{lem}
  Za katerokoli podatkovno vrednost #x# je pričakovana dolžina seznama #t[hash(x)]# največ $#n#_{#x#} + 2$, pri čemer je $#n#_{#x#}$ število pojavitev #x# v razpršeni tabeli.
\end{lem}
...
Sedaj bi želeli dokazati primer \lemref{universal-hashing}, a za slednje, najprej potrebujemo rezultat iz številčne teorije. Pri naslednjem dokazu uporabljamo notacijo 
$(b_r,\ldots,b_0)_2$ pri označevanju $\sum_{i=0}^r b_i2^i$, kjer je vsak $b_i$
bitna vrednost, ali 0 ali 1. Z drugimi besedami je $(b_r,\ldots,b_0)_2$ celo številčna vrednost ali integer, čigar dvojiška predstavitev je podana kot $b_r,\ldots,b_0$.
...
Sledeči izrek prikaže preformanse podatkovne strukture verižena razpršena tabela :


\begin{thm}\thmlabel{hashtable}
  Verižena razpršena tabela uporablja vmesnik #USet#. V kolikor ignoriramo ceno
  klicev na #grow()#, verižena razpršena tabela izvaja operacije #add(x)#,
  #remove(x)#, in #find(x)# v $O(1)$ pričakovanem času na operacijo.

  Če začnemo s prazno veriženo razpršeno tabelo, bo kakršnokoli zaporedje
...
\subsection{Analiza linearnega naslavljanja}
...
\index{run}%
Pravmo da se \emph{izvršitev dolžine $k$, ki se začne pri #i#} zgodi, kadar noben od elementov $#t[i]#, #t[i+1]#,\ldots,#t#[#i#+k-1]$ ni #null# in $#t#[#i#-1]=#t#[#i#+k]=#null#$. Število elementov tabele #t# ki niso #null# je enako #q#, metoda #add(x)# pa zagotavlja, da vedno velja $#q#\le#t.length#/2$. Obstaja #q# elementov $#x#_1,\ldots,#x#_{#q#}$ ki so bili vstavljeni v #t# po zadnji #rebuild()# operaciji. Po naši domnevi ima vsak izmed teh elementov zgočevalno vrednost $#hash#(#x#_j)$, ki je enotna in neodvisna od drugih. S tako nastavitvijo lahko dokažemo glavno trditev potrebno za analiziranje linearnega naslvljanja.

\begin{lem}\lemlabel{linear-probing}
Določi vrednost $#i#\in\{0,\ldots,#t.length#-1\}$. Potem je možnost da se izvršitev dolžine $k$ začne pri #i# enak $O(c^k)$ za konstanto $0<c<1$.
\end{lem}
\translatedby{Klemen Hiti}{sl}
...
Upoštevajte, da ima ta zgoščevalna vrednost zadnji korak (deljenje z #z# in
...
\begin{proof}
  Najprej bomo ignorirali zadnji multiplikativni zgoščevalni korak, in pogledali kako
  ta korak prispeva kasneje.  Opredeli:
  \[
    h'(#x#_0,\ldots,#x#_{r-1}) =  
...
\subsection{Razpršilne funkcije za polja in nize}
\seclabel{polyhash}
...
ne vplivajo na razpršilno funkcijo, ki jo uporabljamo.
...
\begin{thm}
  ...
  \cdots + #x#_{r-1}#z#^{r-1}$ netrivijalni polinom s koeficienti
