...
Množilno razprševanje je učinkovita metoda tvorbe zgoščenih vrednosti osnovana na modularni aritmetiki(opisana v poglavju  \secref{arrayqueue}) in celoštevilskemu deljenju. Uporablja $\ddiv$ operator, 
...
(Glej \figref{multihashing}.) Poleg tega je celoštevilsko deljenje z $2^{#w#-#d#}$ enako izločanju skrajno desnih $#w#-#d#$ bitov v binarni predstavitvi (kar uredimo s premikom $#w#-#d#$ bitov z operatorjem >>>).
S tem dosežemo, da ima koda lažjo implementacijo kot zgornja formula:
